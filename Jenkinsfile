         ...
         stage("Deploying to staging and perform code analysis"){
			parallel{
				stage("Deploy to staging"){
					steps{
						deploy adapters: [tomcat8(credentialsId: 'e96f8189-33d4-4334-b69f-c0e4e00847eb', path: '', url: 'http://ec2-52-14-65-218.us-east-2.compute.amazonaws.com:8888/')], contextPath: null, onFailure: false, war: '**/*.war'
					}
				post{
					success{
					...
					always {
            					emailext body: 'A Test Email', recipientProviders: [developers(), requestor()], subject: 'Jenkins Execution Status', to: 'jiraxray2@gmail.com'
        				}
				}
			}
